...
def ler_configuracao():
    ...
    if dados_config is None:
        ...
        if 'pokeapi-projeto' not in dados or 'treinador-projeto' not in dados or 'pokeapi-url' not in dados or 'treinador-url' not in dados or len(dados) != 4:
...
class SobeServidores:
    ...
    @staticmethod
    def encoding():
        return "windows-1252" if platform.system() == "Windows" else "utf-8"
    ...
    def apocalipse():
        ...
        if platform.system() == "Windows":
            ...
            parts = out.decode(SobeServidores.encoding()).split('\r\n')
            ...
            out = Popen('ps -A', shell = True, stdin = PIPE, stdout = PIPE, stderr = PIPE).communicate()[0]
            parts = out.decode(SobeServidores.encoding()).split('\n')

            for line in parts:
                if 'python' in line and str(meu_pid) not in line:
                    alvo = line[:5]
                    SobeServidores.matar(alvo)
    ...
    def matar(alvo):
        if alvo == -1: return
        comando = f"taskkill /f /pid {alvo}" if platform.system() == "Windows" else f"kill -9 {alvo}"
        tout = Popen(comando, shell = True, stdin = PIPE, stdout = PIPE, stderr = PIPE).communicate()[0]
        print_debug(tout.decode(SobeServidores.encoding()))
    ...
    def spy(ouvido, proc):
        def enqueue_output(out):
            for line in iter(out.readline, b''):
                z = line.decode(SobeServidores.encoding()).replace('\r', '').replace('\n', '')
                ouvido(z)
    ...
    def subir_pokeapi(ouvido):
        pasta = ler_configuracao()["pokeapi-projeto"]
        if pasta == "online": return
        if platform.system() == "Windows":
            comando = f'start "pokeapi" cmd /k python.exe "{pasta}\\manage.py" runserver --settings=config.local'
        else:
            comando = f'xterm -hold -e python "{pasta}/manage.py" runserver --settings=config.local'
        SobeServidores.subir_servidor(ouvido, comando, SobeServidores.pokeapi_online)

    @staticmethod
    def subir_treinador(ouvido):
        pasta = ler_configuracao()["treinador-projeto"]
        if platform.system() == "Windows":
            comando = f'start "treinador" cmd /k python.exe "{pasta}\\treinador.py"'
        else:
            comando = f'xterm -hold -e python "{pasta}/treinador.py"'
        SobeServidores.subir_servidor(ouvido, comando, SobeServidores.treinador_online)

    @staticmethod
    def reset_treinador():
        resposta = api.post(f"{ler_configuracao()['treinador-url']}/reset")
    ...
    def pokeapi_online():
        def crivo(resposta):
            return resposta.json()['pokemon'] == f"{ler_configuracao()['pokeapi-url']}/api/v2/pokemon/"
        return SobeServidores.api_online(f"{ler_configuracao()['pokeapi-url']}/api/v2/", crivo)

    @staticmethod
    def treinador_online():
        def crivo(resposta):
            return resposta.text == 'Pikachu, eu escolho você!'
        return SobeServidores.api_online(f"{ler_configuracao()['treinador-url']}/hello", crivo)

    @staticmethod
    def treinador_pid():
        try:
            resposta = api.get(f"{ler_configuracao()['treinador-url']}/pid")
