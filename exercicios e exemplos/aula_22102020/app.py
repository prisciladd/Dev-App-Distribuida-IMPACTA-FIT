...
class Cliente(db.Model):
    __tablename__ = "tbClientePriscilaDD" #altera nome da classe de Cliente para tbcliente
...
@app.route("/")
def index():
    clientes = Cliente.query.all() #faz select * classe Cliente
    return render_template("index.html", clientes=clientes) #chama index e passa objeto cliente vazio 

@app.route("/add", methods=['GET','POST'])
def add():
    if request.method == 'POST':
        cliente = Cliente(request.form['nome'], request.form['comentario']) #pega o que ta escrito na caixa formulario
